...
If you already cloned the repository without the `--recurse-submodules` flag and need to run the integration tests, you can manually initialize and update the submodules using:

```bash
git submodule update --init --recursive
```

This step is **only necessary if you intend to run the integration tests.**
