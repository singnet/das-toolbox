...
class LogsEvolutionBroker(Command):
    name = "evolution-broker"

    aliases = ["eb"]

    short_help = "Display logs for the Evolution Broker service"

    help = """
'das-cli logs evolution-broker' displays the logs of the Evolution Broker service.
This command retrieves the logs generated by the Evolution Broker, which may include information about evolution processes, agent events, and errors.

.SH EXAMPLES

Display logs of the Evolution Broker service.

$ das-cli logs evolution-broker
"""
    ...
    @ensure_container_running(
        ["_evolution_broker_container_manager"],
        exception_text="Evolution Broker is not running. Please start it with 'das-cli evolution-broker start' before viewing logs.",
        verbose=False,
    )
    def run(self):
        self._settings.raise_on_missing_file()
...
COMMANDS
    ...
    das-cli logs evolution-broker           Logs from the Evolution Broker service
...
EXAMPLES
    ...
    Display logs from the Evolution Broker service:
...
"""
    ...
    def __init__(
        ...
        logs_evolution_broker: LogsEvolutionBroker,
    ) -> None:
        super().__init__()
        self.add_commands(
            [
