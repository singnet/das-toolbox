...
def brokers_port_setup(settings: Settings, broker_name: str):
    ...
    broker_port = Command.prompt(
        ...
        default=extract_service_port(
            str(get_default_value(settings, f"brokers.{broker_name}.endpoint"))
        ),
    )

    broker_port_range = (
        None
        if broker_name == "attention"
        else Command.prompt(
    ...
    if broker_port_range is None:
        return {broker_name: {"endpoint": f"localhost:{broker_port}"}}
