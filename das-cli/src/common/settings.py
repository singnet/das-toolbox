...
class Settings:
    ...
    def raise_on_version_mismatch(self):
        ...
        if self._store.get("schema_version") != base_dict_version:
            mismatch_message = f"Your configuration file doesn't have all the entries this version of das-cli requires. You can call 'das-cli config set' and hit <ENTER> to every prompt in order to re-use the configuration you currently have in your config file and set the new ones to safe default values."
    ...
    def raise_on_missing_file(self):
        if not self.exists():
            raise FileNotFoundError(
                f"No existing configuration path was found. You can run the command `config set` to create a configuration file or point to an existing file."
            )
